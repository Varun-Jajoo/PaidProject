...
export function PortfolioSummary() {
  ...
  const calculatePnL = (position: (typeof portfolio.positions)[0]) => {
    const currentPrice = currentPrices[position.commodity] || position.averagePrice
    return (currentPrice - position.averagePrice) * position.quantity
  }

  const calculatePnLPercentage = (position: (typeof portfolio.positions)[0]) => {
    const currentPrice = currentPrices[position.commodity] || position.averagePrice
              ...
              {portfolio.positions.map((position) => {
                ...
                return (
                  <TableRow key={position.commodity}>
                    <TableCell className="font-medium capitalize">{position.commodity.replace("_", " ")}</TableCell>
