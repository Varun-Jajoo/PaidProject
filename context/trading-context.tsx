...
type TradingContextType = {
  ...
  trades: Trade[]
  watchlist: string[]
  executeTrade: (commodity: string, type: "buy" | "sell", quantity: number, price: number) => Promise<boolean>
...
export function TradingProvider({ children }: { children: React.ReactNode }) {
  ...
  const [watchlist, setWatchlist] = useState<string[]>(["gold", "silver", "crude_oil"])
  ...
  const executeTrade = async (
    ...
    if (type === "sell") {
      const position = portfolio.positions.find((p) => p.commodity === commodity)
    ...
    setPortfolio((prev) => {
      ...
      // Update positions
      const existingPosition = newPortfolio.positions.find((p) => p.commodity === commodity)
      if (existingPosition) {
        if (type === "buy") {
          // Calculate new average price
          const totalValue = existingPosition.quantity * existingPosition.averagePrice + total
          ...
          if (existingPosition.quantity === 0) {
            newPortfolio.positions = newPortfolio.positions.filter((p) => p.commodity !== commodity)
          }
        }
      } else if (type === "buy") {
        // New position
